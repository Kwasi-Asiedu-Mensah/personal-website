...
function getWeatherSurfaceEffects(
  ...
  if (mood === "snow") {
    if (phase === "dawn" || phase === "dusk") return ["cloudBands", "snow"];
    return ["snow"];
  }
  if (phase === "night" && mood === "clear") return ["stars"];
  if (phase === "dawn" || phase === "dusk") return ["cloudBands"];
  if (phase === "day" && mood === "clear") return ["sunGlow"];
  return [];
}
...
export function buildOpenMeteoForecastUrl({
  ...
  if (dailyFields && dailyFields.length > 0) params.set("daily", dailyFields.join(","));
  if (hourlyFields && hourlyFields.length > 0) params.set("hourly", hourlyFields.join(","));
...
export function getWeatherScene(currentTimeIso: string, weatherCode: number): WeatherScene {
  ...
  const theme = WEATHER_THEMES_BY_MOOD[mood][phase];
  const mainEffects = getWeatherSurfaceEffects(phase, mood, "main");
  const previewEffects = getWeatherSurfaceEffects(phase, mood, "preview");
  return {
    key,
    phase,
    mood,
