...
const CLOUD_BAND_TONES: Record<WeatherScene["cloudTone"], [string, string, string]> = {
  ...
  default: ["bg-white/[0.16]", "bg-white/[0.14]", "bg-white/[0.12]"],
};
...
const WEATHER_LIGHTNING_BOLTS = [
  {
    left: "61%", top: "10%", width: "40px", height: "20%", delay: "0s",
    clipPath: "polygon(48% 0%, 68% 0%, 56% 34%, 72% 34%, 38% 68%, 52% 68%, 28% 100%, 40% 62%, 24% 62%)",
  },
  {
    left: "69%", top: "14%", width: "32px", height: "15%", delay: "0.16s",
    clipPath: "polygon(44% 0%, 66% 0%, 54% 38%, 72% 38%, 34% 100%, 46% 62%, 26% 62%)",
  },
  {
    left: "74%", top: "8%", width: "28px", height: "12%", delay: "2.7s",
    clipPath: "polygon(46% 0%, 64% 0%, 54% 34%, 70% 34%, 36% 100%, 46% 64%, 28% 64%)",
  },
] as const;
...
function freezeRainDropsForPreview<
  T extends readonly {
    left: string; top: string; height: number; duration: number;
    delay: number; drift: number; opacity: number;
  }[],
>(drops: T) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0">
      ...
      {hasEffect("cloudDrift") && !isPreview && (
        ...
        >
          <div className="absolute left-[2%] top-[34%] h-20 w-56 rounded-full bg-white/[0.24] blur-xl" />
          <div className="absolute left-[14%] top-[14%] h-28 w-72 rounded-full bg-white/[0.30] blur-xl" />
          <div className="absolute left-[31%] top-[6%] h-32 w-80 rounded-full bg-white/[0.34] blur-xl" />
          <div className="absolute left-[50%] top-[18%] h-28 w-72 rounded-full bg-white/[0.28] blur-xl" />
          <div className="absolute left-[68%] top-[28%] h-24 w-64 rounded-full bg-white/[0.22] blur-xl" />
          <div className="absolute left-[12%] top-[44%] h-16 w-[76%] rounded-full bg-white/[0.18] blur-2xl" />
        </div>
      )}

      {hasEffect("sunGlow") && (
        isPreview ? (
          <div className="absolute right-[8%] top-[8%] h-20 w-20 rounded-full bg-yellow-100/[0.18] blur-2xl" />
        ) : (
          <>
            <div className="absolute -top-14 right-[16%] h-52 w-52 rounded-full bg-yellow-100/[0.18] blur-3xl" />
            <div className="absolute top-4 right-[22%] h-24 w-24 rounded-full bg-yellow-200/[0.30] blur-2xl" />
          </>
        )
      )}
      ...
      {hasEffect("stars") && (
        <div
          className={isPreview ? "absolute inset-0 opacity-[0.68]" : "absolute inset-0 opacity-75"}
          style={{
        ...
        <>
          ...
          <div
            ...
            style={
              isAnimatedCloudyMain
                ? ({
                    ["--cloud-drift-x" as string]: "56px",
                    ["--cloud-drift-y" as string]: "-10px",
                    ["--cloud-drift-duration" as string]: "26s",
                    animationDelay: "-14s",
                  } as CSSProperties)
                : undefined
            }
      ...
      {hasEffect("fog") && (
        <>
          <div
            className={[
              "absolute rounded-full bg-white/[0.22] blur-3xl",
              isPreview
                ? "left-[0%] top-[30%] h-12 w-full"
                : isAnimatedFogMain
                  ? "left-[-10%] top-[28%] h-20 w-[120%]"
                  : "left-[0%] top-[28%] h-16 w-full",
              isAnimatedFogMain && "weather-cloud-drift",
            ].filter(Boolean).join(" ")}
            style={
              isAnimatedFogMain
                ? ({
                    ["--cloud-drift-x" as string]: "64px",
                    ["--cloud-drift-y" as string]: "0px",
          ...
          <div
            className={[
              "absolute rounded-full bg-white/[0.16] blur-3xl",
              isPreview
                ? "left-[0%] bottom-[20%] h-10 w-full"
                : isAnimatedFogMain
                  ? "left-[-8%] bottom-[22%] h-16 w-[114%]"
                  : "left-[0%] bottom-[20%] h-14 w-full",
              isAnimatedFogMain && "weather-cloud-drift",
            ].filter(Boolean).join(" ")}
            style={
              isAnimatedFogMain
                ? ({
                    ["--cloud-drift-x" as string]: "-48px",
                    ["--cloud-drift-y" as string]: "0px",
                    ["--cloud-drift-duration" as string]: "24s",
                    animationDelay: "-10s",
                  } as CSSProperties)
                : undefined
            }
      ...
      {hasEffect("rainDrops") && (
        <div className="absolute inset-0 overflow-hidden">
          {(isPreview ? WEATHER_PREVIEW_RAIN_DROPS : WEATHER_RAIN_DROPS).map((drop, i) => (
            <div
              key={i}
              ...
              style={
                isPreview
                  ? {
                      ...
                      transform: `translateX(${drop.drift}px) rotate(12deg)`,
                    }
                  : ({
                      left: drop.left,
                      top: drop.top,
                      ...
                      height: `${drop.height}px`,
                      opacity: drop.opacity,
                      animation: `weather-rain-drop ${drop.duration}s linear ${drop.delay}s infinite`,
                      ["--rain-drift-x" as string]: `${drop.drift}px`,
                    } as CSSProperties)
              }
            />
          ))}
          {isThunderScene &&
            (isPreview ? WEATHER_PREVIEW_THUNDER_RAIN_DROPS : WEATHER_THUNDER_EXTRA_RAIN_DROPS).map((drop, i) => (
              <div
                key={`thunder-${i}`}
                className="absolute rounded-full bg-white"
                style={
      ...
      {hasEffect("snow") && (
        <div className="absolute inset-0 overflow-hidden">
          {WEATHER_SNOW_FLAKES.map((flake, i) => (
            <div
              key={i}
              className="absolute rounded-full bg-white"
              style={{
                ...
                animation: `weather-snow-flake ${flake.duration}s ease-in-out ${flake.delay}s infinite`,
                ["--snow-drift-x" as string]: `${flake.drift}px`,
              } as CSSProperties}
            />
          ))}
        </div>
      ...
      {hasEffect("lightning") && !isPreview && (
        <div className="absolute inset-0 overflow-hidden">
          {WEATHER_LIGHTNING_BOLTS.map((bolt, i) => (
            <div
              key={i}
              ...
              style={{
                ...
                animation: `weather-lightning 4s ease-in-out ${bolt.delay} infinite`,
                opacity: 0,
              }}
            />
          ))}
